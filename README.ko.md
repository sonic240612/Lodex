...
프로젝트 대화의 계획 패널에서 **명령 실행**을 펼치세요. Docker 명령은 저장한 이미지와 제한 안에서 실행됩니다. 전체 접근은 경고 확인 후 호스트 파일·셸·환경 변수·네트워크를 사용할 수 있습니다. 대화형 PTY는 준비 중입니다.

**Autopilot**은 작업별 완료 기준·검증 명령을 저장하고 모델 요청에 계획 포함을 켠 뒤, 실행 범위와 예산을 정해 시작하세요. 검사 통과와 수동 체크박스는 별도로 기록됩니다. 중단된 실행은 자동 재시작하지 않습니다.
...
명령은 `/ask`, `/status`, `/plan`, `/stop`. 연결된 계정은 전체 접근 대화를 원격 실행할 수 있고, 다른 Build 요청은 Telegram 권한 설정을 따릅니다. Lodex가 실행 중이어야 연결됩니다. 전달 결과를 확인하지 못한 메시지는 자동 재전송하지 않습니다.
