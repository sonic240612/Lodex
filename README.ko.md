...
MCP 설정은 `mcpServers` JSON으로 가져올 수 있습니다. 인증값은 `.env`의 `LODEX_MCP_` 변수로 관리하세요. MCP 도구는 Build 모드에서 실행되며 OpenRouter 전송에는 별도 동의가 필요합니다. OAuth·resources·prompts와 일부 JSON Schema 형식은 준비 중입니다.
