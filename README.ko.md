...
- 프로젝트 파일 단일·묶음 읽기, 재귀 glob 탐색·대소문자 선택 검색, 검토된 경로 생성·이동·삭제, 단일/다중 파일 변경과 Docker 검증 묶음 적용, 텍스트 수정 되돌리기
...
- Eco 모드, 자동 빠른 압축, 현재 대화 기록의 제한된 검색, 대형 도구 결과의 로컬 ObservationPack 보관과 선택적 재호출
...
llama.cpp가 `/props`를 제공하면 현재 Chat template의 도구 호출·system 역할·content·병렬 호출·reasoning 기록 지원 여부를 읽습니다. 요청은 이 능력에 맞게 변환하며, 도구 템플릿이 호환되지 않으면 생성 전에 설정 방법과 함께 중단합니다. 구형 호환 서버는 능력 표시만 미확정 상태로 두고 계속 사용할 수 있습니다.
...
- 압축으로 이전 내용이 빠졌을 때 에이전트가 현재 대화에 저장된 텍스트만 제한된 크기로 검색할 수 있습니다. 다른 대화는 검색하지 않습니다.
