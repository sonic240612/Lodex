...
- 프로젝트 파일 읽기·검색, 단일/다중 파일 변경 검토, Docker 검증과 묶음 적용, 되돌리기
...
MCP 설정은 일반적인 `mcpServers` JSON을 받습니다. 선택한 도구, 정적·매개변수형 텍스트 리소스, 프롬프트, 카탈로그 변경 검사, 비밀 참조, 사전 등록 공개 클라이언트의 OAuth를 지원합니다. MCP 비밀은 `.env`의 `LODEX_MCP_` 이름으로 저장하세요. OAuth 토큰은 Git에서 제외된 `.env.mcp`에 저장됩니다.
...
명령: `/ask`, `/status`, `/plan`, `/stop`. Lodex가 실행 중이어야 합니다. 결과를 확인하지 못한 전달은 기록만 남기고 자동 재시도하지 않습니다.
