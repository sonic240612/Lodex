...
describe('context compiler', () => {
  ...
  it('prepares a tool-free structured LLM handoff while retaining a raw recent tail', () => {
    const source = session();
    source.config = { ...source.config, provider: 'llama-server', model: 'summary-model' };
    source.plan = { ...source.plan, goal: 'Ship semantic compaction', includeInContext: true };
    source.messages = Array.from({ length: 8 }, (_, index) =>
      message(index % 2 ? 'assistant' : 'user', `turn-${index} exact detail`),
    );
    const prepared = prepareSemanticCompaction(source);
    expect(prepared.fallback.method).toBe('fast');
    expect(prepared.fallback.compactedMessageCount).toBe(4);
    expect(prepared.request.tools).toBeUndefined();
    expect(prepared.request.messages[0]?.content).toContain('## Key decisions');
    expect(prepared.request.messages[1]?.content).toContain('Ship semantic compaction');
    expect(prepared.request.messages[1]?.content).toContain('turn-3 exact detail');
    expect(prepared.request.messages[1]?.content).not.toContain('turn-4 exact detail');
    const completed = finishSemanticCompaction(
      prepared.fallback,
      '## Goal\nPreserve the task.\n\n## Progress\nThe checkpoint was generated by a model.',
      'summary-model',
    );
    expect(completed).toMatchObject({ method: 'semantic', model: 'summary-model' });
    source.contextCompaction = completed;
    expect(JSON.stringify(compileContext(source, 'continue').request.messages)).toContain(
      'The checkpoint was generated by a model.',
    );
  });
